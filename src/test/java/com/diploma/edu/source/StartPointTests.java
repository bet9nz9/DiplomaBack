...
class StartPointTests {
	...
	void testNewRequestBuilder(){
		String countMasterValue = "SELECT COUNT(*) FROM ( SELECT o.object_id id,\n" +
				"                              o.name name,\n" +
		...
		String resultRequestMasterValue = "select * from (select row_number() over (order by id ASC) rowRank, a.* from(\n" +
		...
		String resultRequest = Director.valueOf(clazz).getRequest(params2).buildRequest();
		String countResultValue = Director.valueOf(clazz).getRequest(new CountElementsRequest(new Request(clazz), params2)).buildRequest();
		System.out.println();
	}
